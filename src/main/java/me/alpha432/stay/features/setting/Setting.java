...
public class Setting<T> {
    ...
    private Function1<T, T> limiter = t -> t;
    ...
    public void setValue(T value) {
        ...
        ClientChangeEvent.SettingChange<T> settingChange = new ClientChangeEvent.SettingChange<>(this);
        settingChange.post();
        if (!event.isCanceled() && !settingChange.getCancelled()) {
            this.value = this.plannedValue;
        } else {
            this.plannedValue = this.value;
        }
        if (hasLimiter) this.value = limiter.invoke(this.value);
    }
    ...
    public void setLimiter(Function1<T, T> limiter) {
