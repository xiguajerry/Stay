...
public class Scaffold extends Module {
    public Scaffold() {
        super ("Scaffold", "placeholder", Category.MOVEMENT, false, false, false);
    }
    ...
    public Setting<SwingMode> mode = register(new Setting<>("Swing", SwingMode.Mode));
    public Setting<Boolean> swing =  this.register(new Setting<>("Swing Arm", false));
    public Setting<Boolean> bSwitch =  this.register(new Setting<>("Switch", false));
    public Setting<Boolean> center =  this.register(new Setting<>("Center", false));
    public Setting<Boolean> keepY =  this.register(new Setting<>("KeepYLevel", false));
    public Setting<Boolean> sprint =  this.register(new Setting<>("UseSprint", true));
    public Setting<Boolean> replenishBlocks =  this.register(new Setting<>("ReplenishBlocks", true));
    public Setting<Boolean> down = this.register(new Setting<>("Down", false));
    public Setting<Integer> expand =register(new Setting<>("Expand",1,1,6));
    ...
    public void onUpdate() {
        ...
        if (replenishBlocks.getValue() && !(mc.player.getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemBlock) && getBlockCountHotbar() <= 0 ) {
            for (int i = 9; i < 45; ++i) {
        ...
        if (blockData != null) {
            ...
            if (mode.getValue() == SwingMode.Mode) {
