...
public class Skeleton extends Module {
    ...
    public Skeleton() {
        super("Skeleton", "Draws a skeleton inside the player.", Module.Category.VISUAL, false, false, false);
    }
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        Skeleton.mc.world.playerEntities.forEach(e -> drawSkeleton(event, e));
    ...
    private void drawSkeleton(Render3DEvent event, EntityPlayer e) {
        ...
        if (e.isInvisible() && !invisibles.getValue().booleanValue()) {
            return;
        }
        ...
        if (entPos != null && e.isEntityAlive() && !e.isDead && e != Skeleton.mc.player && !e.isPlayerSleeping()) {
            ...
            if (Stay.friendManager.isFriend(e.getName())) {
