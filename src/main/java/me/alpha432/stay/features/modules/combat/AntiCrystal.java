...
public class AntiCrystal
    ...
    public Setting<Float> range = this.register(new Setting<>("Range", Float.valueOf(6.0f), Float.valueOf(0.0f), Float.valueOf(10.0f)));
    public Setting<Float> wallsRange = this.register(new Setting<>("WallsRange", Float.valueOf(3.5f), Float.valueOf(0.0f), Float.valueOf(10.0f)));
    public Setting<Float> minDmg = this.register(new Setting<>("MinDmg", Float.valueOf(6.0f), Float.valueOf(0.0f), Float.valueOf(100.0f)));
    public Setting<Float> selfDmg = this.register(new Setting<>("SelfDmg", Float.valueOf(2.0f), Float.valueOf(0.0f), Float.valueOf(10.0f)));
    ...
    public Entity getBreakTarget(Entity deadlyCrystal) {
        ...
        for (Entity entity : AntiCrystal.mc.world.loadedEntityList) {
            float damage;
            if (!(entity instanceof EntityEnderCrystal) || (damage = DamageUtil.calculateDamage(entity, AntiCrystal.mc.player)) > this.selfDmg.getValue().floatValue() || entity.getDistanceSq(deadlyCrystal) > 144.0 || AntiCrystal.mc.player.getDistanceSq(entity) > MathUtil.square(this.wallsRange.getValue().floatValue()) && EntityUtils.rayTraceHitCheck(entity, true))
                continue;
