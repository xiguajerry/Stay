...
public class LongJump
    ...
    private final Setting<Float> boost = this.register(new Setting<>("Boost", Float.valueOf(4.48f), Float.valueOf(1.0f), Float.valueOf(20.0f)));
    private final Setting<Mode> mode = this.register(new Setting<>("Mode", Mode.DIRECT));
    private final Setting<Boolean> lagOff = this.register(new Setting<>("LagOff", false));
    private final Setting<Boolean> autoOff = this.register(new Setting<>("AutoOff", false));
    private final Setting<Boolean> disableStrafe = this.register(new Setting<>("DisableStrafe", false));
    private final Setting<Boolean> strafeOff = this.register(new Setting<>("StrafeOff", false));
    private final Setting<Boolean> step = this.register(new Setting<>("SetStep", false));
    ...
    public void onDisable() {
        Stay.timerManager.setTimer(1.0f);
    }
    ...
    private void doNormal(final MotionUpdateEvent event) {
        ...
        switch (this.mode.getValue()) {
            ...
            case DIRECT: {
                if (EntityUtils.isInLiquid() || EntityUtils.isOnLiquid()) {
                    break;
                }
                ...
                if (!LongJump.mc.player.collidedVertically) {
                    ...
                    Stay.timerManager.setTimer(0.85f);
    ...
    private void doVirtue(final MoveEvent event) {
        if (this.mode.getValue() == Mode.VIRTUE && (LongJump.mc.player.moveForward != 0.0f || (LongJump.mc.player.moveStrafing != 0.0f && !EntityUtils.isOnLiquid() && !EntityUtils.isInLiquid()))) {
            if (this.stage == 0) {
