...
import me.alpha432.stay.features.modules.player.XCarry;
...
public class AutoArmor
    ...
    private final Setting<Integer> closestEnemy = this.register(new Setting<>("Enemy", Integer.valueOf(8), Integer.valueOf(1), Integer.valueOf(20), v -> this.mendingTakeOff.getValue()));
    private final Setting<Integer> helmetThreshold = this.register(new Setting<>("Helmet%", Integer.valueOf(80), Integer.valueOf(1), Integer.valueOf(100), v -> this.mendingTakeOff.getValue()));
    private final Setting<Integer> chestThreshold = this.register(new Setting<>("Chest%", Integer.valueOf(80), Integer.valueOf(1), Integer.valueOf(100), v -> this.mendingTakeOff.getValue()));
    private final Setting<Integer> legThreshold = this.register(new Setting<>("Legs%", Integer.valueOf(80), Integer.valueOf(1), Integer.valueOf(100), v -> this.mendingTakeOff.getValue()));
    private final Setting<Integer> bootsThreshold = this.register(new Setting<>("Boots%", Integer.valueOf(80), Integer.valueOf(1), Integer.valueOf(100), v -> this.mendingTakeOff.getValue()));
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(AutoArmor.mc.currentScreen instanceof StayGui) && this.elytraBind.getValue().getKey() == Keyboard.getEventKey()) {
    ...
    public void onTick() {
        ...
        if (this.taskList.isEmpty()) {
            ...
            ItemStack chest;
            int slot4;
            if (this.mendingTakeOff.getValue().booleanValue() && InventoryUtil.holdingItem(ItemExpBottle.class) && AutoArmor.mc.gameSettings.keyBindUseItem.isKeyDown() && (this.isSafe() || EntityUtils.isSafe((Entity)AutoArmor.mc.player, 1, false, true))) {
                int bootDamage;
                int leggingDamage;
                int chestDamage;
                int helmDamage;
        ...
        if (this.timer.passedMs((int)((float)this.delay.getValue().intValue() * (this.tps.getValue() != false ? Stay.serverManager.getTpsFactor() : 1.0f)))) {
            if (!this.taskList.isEmpty()) {
    ...
    private boolean isSafe() {
        EntityPlayer closest = EntityUtils.getClosestEnemy(this.closestEnemy.getValue().intValue());
        if (closest == null) {
            return true;
        }
