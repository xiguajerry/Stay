...
import me.alpha432.stay.manager.FriendManager;
    ...
    public void execute(String[] commands) {
        if (commands.length == 1) {
            if (Stay.friendManager.getFriends().isEmpty()) {
                ...
                String f = "Friends: ";
                for (FriendManager.Friend friend : Stay.friendManager.getFriends()) {
                    try {
        ...
        if (commands.length == 2) {
            ...
            FriendCommand.sendMessage(commands[0] + (Stay.friendManager.isFriend(commands[0]) ? " is friended." : " isn't friended."));
            return;
        }
        if (commands.length >= 2) {
            switch (commands[0]) {
