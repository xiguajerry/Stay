...
public class BlockUtil
    ...
    public static boolean canBlockBeSeen(double x, double y, double z) {
        if (BlockUtil.mc.world.rayTraceBlocks(new Vec3d(BlockUtil.mc.player.posX, BlockUtil.mc.player.posY + (double)BlockUtil.mc.player.getEyeHeight(), BlockUtil.mc.player.posZ), new Vec3d(x, y + 1.7, z), false, true, false) != null) return false;
        return true;
    }
    public static EnumFacing getPlaceableSideExlude(BlockPos blockPos, ArrayList<EnumFacing> arrayList) {
        ...
        for(int var4 = 0; var4 < var3; ++var4) {
            ...
            BlockPos blockPos2;
            if (!arrayList.contains(enumFacing) && mc.world.getBlockState(blockPos2 = blockPos.offset(enumFacing)).getBlock().canCollideCheck(mc.world.getBlockState(blockPos2), false) && !mc.world.getBlockState(blockPos2).getMaterial().isReplaceable()) {
                return enumFacing;
            }
        }
    ...
    public static void faceVectorPacketInstant(Vec3d vec3d, Boolean bl) {
        float[] fArray = getNeededRotations2(vec3d);
    ...
    public static List<BlockPos> getBlockSphere(float breakRange, Class clazz) {
        ...
        positions.addAll(BlockUtil.getSphere(EntityUtils.getPlayerPos(BlockUtil.mc.player), breakRange, (int) breakRange, false, true, 0).stream().filter(pos -> clazz.isInstance(BlockUtil.mc.world.getBlockState(pos).getBlock())).collect(Collectors.toList()));
        return positions;
    }

    public static int getenum(BlockPos pos1,BlockPos pos2){
    ...
    public static boolean placeBlockSmartRotate(BlockPos pos, EnumHand hand, boolean rotate, boolean packet, boolean isSneaking) {
        ...
        if (rotate) {
            Stay.rotationManager.lookAtVec3d(hitVec);
        }
        BlockUtil.rightClickBlock(neighbour, hitVec, hand, opposite, packet);
        BlockUtil.mc.player.swingArm(EnumHand.MAIN_HAND);
        BlockUtil.mc.rightClickDelayTimer = 4;
    ...
    public static List<BlockPos> possiblePlacePositions(float placeRange) {
        NonNullList positions = NonNullList.create();
        positions.addAll(BlockUtil.getSphere(EntityUtils.getPlayerPos(BlockUtil.mc.player), placeRange, (int) placeRange, false, true, 0).stream().filter(BlockUtil::canPlaceCrystal).collect(Collectors.toList()));
        return positions;
    }
    ...
    public static List<BlockPos> possiblePlacePositions(float placeRange, boolean specialEntityCheck, boolean oneDot15) {
        NonNullList positions = NonNullList.create();
        positions.addAll(BlockUtil.getSphere(EntityUtils.getPlayerPos(BlockUtil.mc.player), placeRange, (int) placeRange, false, true, 0).stream().filter(pos -> BlockUtil.canPlaceCrystal(pos, specialEntityCheck, oneDot15)).collect(Collectors.toList()));
        return positions;
    }
    ...
    public static void placeCrystalOnBlock(BlockPos pos, EnumHand hand) {
        BlockUtil.mc.player.connection.sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(pos, EnumFacing.UP, hand, (float)pos.getX(), (float)pos.getY(), (float)pos.getZ()));
    }
