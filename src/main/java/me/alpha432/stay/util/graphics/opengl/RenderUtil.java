...
public class RenderUtil implements Util {
    ...
    public static void drawGradientPlane(final BlockPos pos, final EnumFacing face, final Color startColor, final Color endColor, final boolean half, final boolean top) {
        ...
        final Vec3d interp = EntityUtils.interpolateEntity((Entity) RenderUtil.mc.player, RenderUtil.mc.getRenderPartialTicks());
    ...
    public static void drawBoxESP(final Vec3d pos, final Color color, final boolean secondC, final Color secondColor, final float lineWidth, final boolean outline, final boolean box, final int boxAlpha) {
        if (box) {
            drawBox(pos, new Color(color.getRed(), color.getGreen(), color.getBlue(), boxAlpha));   }
        if (outline) {
            drawBlockOutline(pos,secondC ? secondColor : color,lineWidth,true);
        }
    }
    public static void drawBoxESP(final Vec3d pos, final Color color, final int boxAlpha) {
    ...
    public static void drawBoundingBoxTest(AxisAlignedBB aa) {
        final Tessellator tessellator = Tessellator.getInstance();
    ...
    public static void drawBox(final Vec3d pos, final Color color) {
        final AxisAlignedBB bb = new AxisAlignedBB(pos.x - RenderUtil.mc.getRenderManager().viewerPosX, pos.y - RenderUtil.mc.getRenderManager().viewerPosY, pos.z- RenderUtil.mc.getRenderManager().viewerPosZ, pos.x + 1 - RenderUtil.mc.getRenderManager().viewerPosX, pos.y + 1 - RenderUtil.mc.getRenderManager().viewerPosY, pos.z+ 1 - RenderUtil.mc.getRenderManager().viewerPosZ);
        RenderUtil.camera.setPosition(Objects.requireNonNull(RenderUtil.mc.getRenderViewEntity()).posX, RenderUtil.mc.getRenderViewEntity().posY, RenderUtil.mc.getRenderViewEntity().posZ);
        if (RenderUtil.camera.isBoundingBoxInFrustum(new AxisAlignedBB(bb.minX + RenderUtil.mc.getRenderManager().viewerPosX, bb.minY + RenderUtil.mc.getRenderManager().viewerPosY, bb.minZ + RenderUtil.mc.getRenderManager().viewerPosZ, bb.maxX + RenderUtil.mc.getRenderManager().viewerPosX, bb.maxY + RenderUtil.mc.getRenderManager().viewerPosY, bb.maxZ + RenderUtil.mc.getRenderManager().viewerPosZ))) {
            GlStateManager.pushMatrix();
            GlStateManager.enableBlend();
    ...
    public static void drawBlockOutline(final Vec3d pos, final Color color, final float linewidth, final boolean air) {
        final AxisAlignedBB bb = new AxisAlignedBB(pos.x - RenderUtil.mc.getRenderManager().viewerPosX, pos.y - RenderUtil.mc.getRenderManager().viewerPosY, pos.z- RenderUtil.mc.getRenderManager().viewerPosZ, pos.x + 1 - RenderUtil.mc.getRenderManager().viewerPosX, pos.y + 1 - RenderUtil.mc.getRenderManager().viewerPosY, pos.z+ 1 - RenderUtil.mc.getRenderManager().viewerPosZ);
    ...
    public static void drawText(final BlockPos pos, final String text) {
        ...
        GlStateManager.pushMatrix();
        glBillboardDistanceScaled(pos.getX() + 0.5f, pos.getY() + 0.5f, pos.getZ() + 0.5f, RenderUtil.mc.player, 1.0f);
        GlStateManager.disableDepth();
        GlStateManager.translate(-(Stay.textManager.getStringWidth(text) / 2.0), 0.0, 0.0);
        Stay.textManager.drawStringWithShadow(text, 0.0f, 0.0f, -5592406);
        GlStateManager.popMatrix();
    }

    @Contract(pure = true)
    public static void drawText(@NotNull AxisAlignedBB bb, @NotNull final String text) {
        GlStateManager.pushMatrix();
        glBillboardDistanceScaled((float) ((bb.maxX - bb.minX) / 2f + bb.minX), (float) ((bb.maxY - bb.minY) / 2 + bb.minY), (float) ((bb.maxZ - bb.minZ) / 2 + bb.minZ), RenderUtil.mc.player, 1.0f);
        GlStateManager.disableDepth();
        assert Stay.textManager != null;
        GlStateManager.translate(-(Stay.textManager.getStringWidth(text) / 2.0), 0.0, 0.0);
        Stay.textManager.drawStringWithShadow(text, 0.0f, 0.0f, -5592406);
        GlStateManager.popMatrix();
    }
